...
public class BuyerCartServiceImpl implements BuyerCartService {
    ...
    public List<BuyerCart> addItemToCartList(List<BuyerCart> cartList, Long itemId, Integer num) {
        ...
        BuyerCart buyerCart = seachCartBySellerId(cartList, sellerId);
        ...
        if (buyerCart == null) {
            ...
            OrderItem orderItem = seachOrderItemByItemId(orderItemList, itemId);
            //6.b.2判断购物车明细是否为空
            if (orderItem == null) {
                //6.b.3为空，新增购物车明细
    ...
    /**
     * 查找购物项集合中指定库存id的购物项对象 有就返回没有就返回null
     *
     * @param OrderItemList 购物项集合
     * @param itemId        库存id
     * @return
     */
    private OrderItem seachOrderItemByItemId(List<OrderItem> OrderItemList, Long itemId) {
    ...
    /**
     * 在购物车集合中找到指定卖家的购物车对象返回,如果找不到就返回null
     *
     * @param cartList 购物车集合
     * @param sellerId 卖家id
     * @return
     */
    private BuyerCart seachCartBySellerId(List<BuyerCart> cartList, String sellerId) {
        if (cartList != null) {
